...
/**
 * @brief Cycle through a list of single-ended input channels (SING_x constants).
 * @param muxList Pointer to array of SING_x constants (e.g. SING_0, SING_7, ...).
 * @param muxCount Number of entries in muxList. If 0, defaults to all single-ended channels.
 * If muxList is nullptr, defaults to all single-ended channels (SING_0 to SING_7).
 * This function cycles through the provided list of single-ended channels,
 * updating the MUX register for each channel and acquiring a value.
 * It can be used for continuous acquisition of multiple channels in a single call.
 * The function will store the provided muxList and reuse it in subsequent calls
 * until a new list is provided or the acquisition is stopped.*
 * @return Last acquired value.
 */
long ADS1256::cycleSingle(const uint8_t *muxList, size_t muxCount)
{
	// Default to all single-ended channels if no list is provided
	static const uint8_t defaultMuxList[8] = {
		SING_0, SING_1, SING_2, SING_3, SING_4, SING_5, SING_6, SING_7};

	// Static storage for the active mux list and count
	static uint8_t storedMuxList[8];
	static size_t storedMuxCount = 0;
	static size_t cycleIdx = 0;

	if (_isAcquisitionRunning == false)
	{
		// If no list provided, use default
		if (muxList == nullptr || muxCount == 0)
		{
			muxList = defaultMuxList;
			muxCount = 8;
		}
		// Store the mux list for subsequent calls
		if (muxCount > 8)
			muxCount = 8; // Prevent overflow
		memcpy(storedMuxList, muxList, muxCount);
		storedMuxCount = muxCount;
		cycleIdx = 0;
		...
		_spi->transfer(0x00);
		_spi->transfer(storedMuxList[cycleIdx]); // Set initial MUX
		digitalWrite(_CS_pin, HIGH);
		delay(50);
		digitalWrite(_CS_pin, LOW);
	}
	...
	{
		// If running and no arguments, use stored list
		if (muxList == nullptr || muxCount == 0)
		{
			muxList = storedMuxList;
			muxCount = storedMuxCount;
		}
		// If running and arguments are given, ignore them (keep using stored list)
		else
		{
			muxList = storedMuxList;
			muxCount = storedMuxCount;
	...
	// Step 1. - Updating MUX to current channel
	updateMUX(storedMuxList[cycleIdx]);
	...
	// Step 3.
	_spi->transfer(0b00000001); // Issue RDATA (0000 0001) command
	delayMicroseconds(7);		// Wait t6 time
...
long ADS1256::cycleDifferential(int8_t stepwidth /* = 1 */) // Cycling through the differential inputs (AINx + AINy) with a channel step width of stepwidth (e.g. stepwidth = 2 means AIN0+AIN1, AIN4+AIN5, AIN0+AIN1, AIN4+AIN5, etc.)
{
	if (_isAcquisitionRunning == false)
	{
		_cycle = 0;
	...
	if (_cycle < 4)
	{
		...
		_cycle += stepwidth; // Increase cycle - This will move to the next MUX input channel
		if (_cycle >= 4)
		{
			_cycle = 0;
