...
public:
	...
	long cycleSingle(const uint8_t* muxList, size_t muxCount); // Ax + COM, cycles in steps of stepwidth (e.g. stepwidth = 7 means AIN0, AIN7, AIN0, AIN7, etc.)
	// Cycling through the differential inputs
	long cycleDifferential(int8_t stepwidth = 1); // Ax + Ay, cycles in steps of stepwidth (e.g. stepwidth = 2 means AIN0 + AIN1, AIN4 + AIN5, AIN0 + AIN1, AIN4 + AIN5, etc.)
